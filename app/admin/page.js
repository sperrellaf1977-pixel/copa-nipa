          ...
          {TABS.map((t) => (
            <button
              ...
              className={`flex-1 rounded-xl py-2.5 text-sm font-semibold transition ${
                tab === t ? "bg-[#1a8ad8] text-white" : "text-white/50 hover:text-white"
              }`}
            >
              {t}
            </button>
...
function TabInscricoes() {
  ...
  async function load() {
    setLoading(true);
    const { data } = await supabase.from("registrations").select("*").order("id", { ascending: false });
  ...
  const statusColor = (s) =>
    ...
    s === "cancelado" ? "bg-red-500/20 text-red-300" :
  ...
  return (
    <div>
      <div className="mb-6 grid gap-4 sm:grid-cols-3">
        {[
          ["Total", registrations.length, "text-white"],
          ["Pagos", paid, "text-green-400"],
          ["Pendentes", registrations.length - paid, "text-yellow-400"],
        ].map(([l, v, c]) => (
            ...
            <div key={r.id} className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <button
                  ...
                  className={`rounded-xl px-3 py-1.5 text-xs font-semibold ${statusColor(r.payment_status || "pendente")}`}
                >
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                {teams.map((t) => (
                  <button
                    key={t}
                    onClick={() => updateTeam(r, t)}
                    className={`rounded-lg px-2.5 py-1 text-xs transition ${
                      r.team === t ? "bg-[#1a8ad8] text-white" : "border border-white/10 text-white/50 hover:border-white/20"
                    }`}
                  >
                    {t}
                  </button>
                ...
                {r.team && (
                  <button onClick={() => updateTeam(r, null)} className="rounded-lg border border-red-500/20 px-2.5 py-1 text-xs text-red-400 hover:bg-red-500/10">
                    Remover
                  </button>
                )}
              </div>
  ...
  return (
    <div>
      <p className="mb-6 text-sm text-white/50">
        Clique em <strong className="text-white">Editar</strong> para atualizar o placar. A classificação é recalculada automaticamente.
      </p>
                  ...
                  {editing === match.id ? (
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      <input
                        ...
                        className="w-16 rounded-xl border border-white/10 bg-black/30 px-3 py-2 text-center text-sm outline-none"
                        placeholder="0"
                      />
                      ...
                      <button onClick={() => setEditing(null)} className="rounded-xl border border-white/10 px-4 py-2 text-sm text-white/50">
                        Cancelar
                      </button>
                    </div>
                  ) : (
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      ...
                      <div className="flex items-center gap-3">
                        <span className={`rounded-lg px-2.5 py-1 text-xs ${
                          match.status === "Finalizado" ? "bg-green-500/20 text-green-300" :
                          match.status === "Grande Final" ? "bg-yellow-500/20 text-yellow-300" :
                          "bg-white/5 text-white/40"
                        }`}>{match.status}</span>
                        <button onClick={() => startEdit(match)} className="rounded-xl border border-white/10 px-3 py-1.5 text-xs text-white/60 hover:bg-white/5">
                          Editar
                        </button>
                      </div>
                    </div>
      ...
      <div className="mb-8 rounded-2xl border border-white/10 bg-white/[0.03] p-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            ...
            <select
              value={form.round_title}
              onChange={(e) => setForm({ ...form, round_title: e.target.value })}
          ...
          <div>
            ...
            <input
              value={form.round_subtitle}
              onChange={(e) => setForm({ ...form, round_subtitle: e.target.value })}
        ...
        <div className="mt-4">
          ...
          <input type="file" accept="image/*" onChange={handleFile} className="text-sm text-white/60 file:mr-3 file:rounded-xl file:border-0 file:bg-[#1a8ad8] file:px-4 file:py-2 file:text-sm file:text-white" />
        </div>

        {preview && (
          <img src={preview} alt="Preview" className="mt-4 h-40 rounded-xl object-cover" />
        )}

        <button
          onClick={upload}
          disabled={uploading || !form.file}
          className="mt-4 rounded-xl bg-[#1a8ad8] px-6 py-2.5 text-sm font-semibold disabled:opacity-50"
        >
          ...
          {photos.map((photo) => (
            <div key={photo.id} className="overflow-hidden rounded-2xl border border-white/10 bg-white/[0.03]">
              ...
              <div className="flex items-center justify-between p-3">
                ...
                <button onClick={() => deletePhoto(photo)} className="rounded-lg border border-red-500/20 px-2.5 py-1 text-xs text-red-400 hover:bg-red-500/10">
                  Deletar
                </button>
              </div>
            </div>
